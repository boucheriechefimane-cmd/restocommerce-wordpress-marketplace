...
function restocommerce_whatsapp_message( WC_Order $order ) : string {
	...
	$lines[] = '';
	$lines[] = sprintf( __( 'Total : %s', 'restocommerce-whatsapp' ), wp_strip_all_tags( $order->get_formatted_order_total() ) );
...
function restocommerce_render_whatsapp_cta( $order_id ) : void {
	...
	printf( '<p><a class="button alt" href="%1$s" target="_blank" rel="noopener noreferrer">%2$s</a></p>', esc_url( $url ), esc_html__( 'Confirmer la commande sur WhatsApp', 'restocommerce-whatsapp' ) );
}
