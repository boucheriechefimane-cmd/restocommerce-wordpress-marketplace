...
const openOnboarding = async (page) => { const dialog = page.locator('[data-rc-vendor-onboarding][open]'); await dialog.waitFor({ state: 'visible', timeout: 1200 }).catch(async () => { await page.locator('[data-rc-open-onboarding]:visible').first().click(); }); await dialog.waitFor({ state: 'visible', timeout: 15000 }); };
...
const openProductWizard = async (page) => { const dialog = page.locator('[data-rc-product-wizard][open]'); await dialog.waitFor({ state: 'visible', timeout: 1200 }).catch(async () => { await page.locator('[data-rc-tab="menu"]:visible').first().click(); await page.locator('[data-rc-open-product-wizard]:visible').first().click(); }); await dialog.waitFor({ state: 'visible', timeout: 15000 }); };
const addFirstDish = async (page) => {
  ...
  await page.waitForLoadState('domcontentloaded'); await page.locator('[data-rc-vendor-app]').waitFor({ state: 'visible', timeout: 30000 });
};
...
try {
  ...
  await page.locator('[data-rc-onboarding-name]').fill(shopName); await onboardingNext(page); await page.locator('[data-rc-onboarding-city]').waitFor({ state: 'visible' });
